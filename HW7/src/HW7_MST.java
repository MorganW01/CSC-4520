//this program implements Prim's algorithm in Java

public class HW7_MST {
    ...
    void showMinimumSpanningTree(int[] mstArray, int[][] graphArray) {
        ...
        for (int j = 1; j < countOfVertices; j++)
            System.out.println(mstArray[j] + " TO " + j + "\t \t" + graphArray[j][mstArray[j]]);
    }
    ...
    void designMST(int[][] graphArray) {
        ...
        // set value 0 to the 1st vertex because first vertex is always included in MST.
        ...
        for (int i = 0; i < countOfVertices - 1; i++) {
            // selects the vertex having minimum key and that is not added in the MST yet from the set of vertices
            int edge = findMinKeyVertex(keys, setOfMST);
            // Add the selected minimum key vertex to the setOfMST
            setOfMST[edge] = true;
            // changes the key value and the parent index of all the adjacent vertices of the selected vertex. The vertices that are not yet included in MST are considered.

            for (int vertex = 0; vertex < countOfVertices; vertex++)
                // The value of the graphArray[edge][vertex] is non zero only for adjacent vertices of m setOfMST[vertex] is false for vertices not yet included in MST
                // when the value of the graphArray[edge][vertex] is LESS than key[vertex], we update the key
    ...
    public static void main(String[] args) {
        ...
        // this line prints the MST solution
